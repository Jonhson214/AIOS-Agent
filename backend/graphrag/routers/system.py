...
_GRAPHRAG_ROOT = Path(__file__).resolve().parents[1]


def _default_mineru_python() -> Path:
    win = _GRAPHRAG_ROOT / "mineru_mvp" / ".venv" / "Scripts" / "python.exe"
    nix = _GRAPHRAG_ROOT / "mineru_mvp" / ".venv" / "bin" / "python"
    return win if win.exists() else nix


def _default_langextract_python() -> Path:
    win = _GRAPHRAG_ROOT / "langextract_src" / ".venv" / "Scripts" / "python.exe"
    nix = _GRAPHRAG_ROOT / "langextract_src" / ".venv" / "bin" / "python"
    return win if win.exists() else nix
...
async def health_check():
    ...
    mineru_python = Path(os.getenv("MINERU_PYTHON", str(_default_mineru_python())))
    langextract_python = Path(os.getenv("LANGEXTRACT_PYTHON", str(_default_langextract_python())))
...
async def get_demo_data():
    ...
    if not nodes:
        ...
        legacy_nodes_path = _GRAPHRAG_ROOT / "graphrag_pipeline" / "output" / "kg_nodes.json"
        legacy_edges_path = _GRAPHRAG_ROOT / "graphrag_pipeline" / "output" / "kg_edges.json"
